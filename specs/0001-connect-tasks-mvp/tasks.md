...
- [x] T001 Extend the vitest coverage allowlist in `vitest.config.ts` with
...
- [x] T002 [US1] RED: `src/services/syno-errors.test.ts` — every ApiError code
...
- [x] T003 [US1] GREEN: implement `src/services/syno-errors.ts` until T002
      passes; refactor `LoginPage.vue` to use it (drop the inline map).
- [x] T004 [US1] RED→GREEN: `e2e/connect.spec.ts` — wrong-password error,
...
- [x] T005 [US2] RED→GREEN: `e2e/tasks-list.spec.ts` — field-level assertions
...
- [x] T006 [P] [US3] RED: `src/services/url-detect.test.ts` — extracts
...
- [x] T007 [US3] GREEN: implement `src/services/url-detect.ts` until T006
      passes.
- [x] T008 [US3] Build `src/components/FolderPickerModal.vue` — shares →
      subfolder drill-down over `/v1/fs`, breadcrumb back navigation, radio
      selection; returns the picked share-relative destination.
- [x] T009 [US3] Build `src/components/NewTaskModal.vue` — URL textarea with
      ...
      it.
- [x] T010 [US3] RED→GREEN: `e2e/add-task.spec.ts` — multi-URL create lands
...
- [x] T011 [US4] Extend `src/components/TaskItem.vue` with ion-item-sliding:
      ...
      refresh.
- [x] T012 [US4] RED→GREEN: `e2e/task-actions.spec.ts` — pause a downloading
...
- [x] T013 [P] [US5] RED: `src/services/task-sort.test.ts` — each sort key
...
- [x] T014 [US5] GREEN: implement `src/services/task-sort.ts` until T013
      passes.
- [x] T015 [US5] Build `src/composables/useTaskFilter.ts` — filter state
      persisted as the `taskFilter` row of the idb `settings` store (defaults:
      createdAt desc, all statuses on, empty term).
- [x] T016 [US5] Build `src/components/TaskFilterSheet.vue` — term input, sort
...
- [x] T017 [US5] RED→GREEN: `e2e/filter-sort.spec.ts` — size sort both
      directions, term narrows, unchecked status hides, choices survive a
      reload.

## Phase 8: Polish & gates

- [x] T018 Checklist pass: complete `checklists/credential-safety.md`
      (Principle III — spec touches login credentials + source credentials).
- [x] T019 Full gate: `npm run build`, `npm run test:unit:coverage` (new
      floors green), `cd server && go build ./... && go vet ./... && go test
      ./...`, `npm run test:e2e` (all six e2e specs).
- [x] T020 Bump spec Status → `in-review`, run `make roadmap`, commit the

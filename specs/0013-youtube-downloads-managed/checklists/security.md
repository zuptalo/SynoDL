...
- [x] CHK028 Is ownership specified for the artwork proxy, or is it stated why that endpoint is exempt? [Gap — the proxy takes a caller-supplied URL and is not covered by FR-007 or FR-008] **→ spec updated:** FR-009d added. Revisited during implementation: gating it was the wrong answer. The caller supplies the address, the content is public, and image requests carry no session — so a session check would have cost caching or leaked a token into URLs to protect nothing. FR-009d now states the property that matters (no listing, no enumeration, host allowlist as the control) and is tested as such.

...
test("multi-URL input counts links, creates one task per URL with the picked destination", async ({
  page,
}) => {
  await login(page);
  ...
  await page
    ...
    .locator("textarea")
    .fill(
      "http://mirror.example/one.iso\nnot a url line\nmagnet:?xt=urn:btih:abc123",
    );
  await expect(page.getByTestId("newtask-count")).toHaveText(
    "2 links detected",
  );
  ...
  await expect(page.getByTestId("newtask-destination")).toContainText(
    "tv-show/Friends",
  );
...
test("a large mixed-delimiter paste is parsed and added in batches of ten", async ({
  page,
}) => {
  await login(page);
  await openNewTask(page);
  ...
  const urls = Array.from(
    { length: 12 },
    (_, i) => `http://mirror.example/file${i}.iso`,
  );
  const blob =
    urls.slice(0, 5).join(", ") +
    "; " +
...
test("cancel leaves the destination picker without changing the destination", async ({
  page,
}) => {
  await login(page);
  await openNewTask(page);
...
test("create a subfolder inside a folder and select it as the destination", async ({
  page,
}) => {
  await login(page);
  await openNewTask(page);
...
test("the destination picker reopens inside the current destination, not root", async ({
  page,
}) => {
  await login(page);
  await openNewTask(page);
...
test("create a folder in the current destination right from the task screen", async ({
  page,
}) => {
  await login(page);
  await openNewTask(page);
...
test("the folder picker search filters by a partial name match anywhere", async ({
  page,
}) => {
  await login(page);
  await openNewTask(page);
...
test("favoriting a folder gives a one-tap quick-select chip", async ({
  page,
}) => {
  await login(page);
  await openNewTask(page);
...
test("paste appends bulk URLs (never glued into one) and Clear empties the box", async ({
  ...
  // First bulk of 2.
  await page.evaluate(() =>
    navigator.clipboard.writeText("http://a/1.iso\nhttp://a/2.iso"),
  );
  ...
  await page.evaluate(() =>
    navigator.clipboard.writeText(
      "http://a/3.iso http://a/4.iso, http://a/5.iso",
    ),
  );
  await page.getByTestId("newtask-paste").click();
  await expect(page.getByTestId("newtask-count")).toHaveText(
...
test("junk-only input keeps the confirm button disabled", async ({ page }) => {
  ...
  await expect(page.getByTestId("newtask-submit")).toHaveAttribute(
    "aria-disabled",
    "true",
  );
});

test("a .torrent upload becomes a task named after the file", async ({
  page,
}) => {
  await login(page);
  await openNewTask(page);
  ...
  await expect(
    page.getByTestId("task-item").filter({ hasText: "great-distro.torrent" }),
  ).toHaveCount(1);
});

test("an oversized torrent is refused with a clear message", async ({
  page,
}) => {
  await login(page);
  await openNewTask(page);

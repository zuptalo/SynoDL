...
test("tapping a row opens the detail sheet with full fields", async ({
  page,
}) => {
  await seedTasks([
    {
  ...
  await expect(page.getByTestId("detail-destination")).toContainText(
    "home/Downloads",
  );
...
test("detail shows the source link and re-downloads a finished task", async ({
  page,
}) => {
  await seedTasks([
    {
  ...
  await page.getByTestId("task-item").click();
  await expect(page.getByTestId("detail-uri")).toContainText(
    "http://mirror.example/done.iso",
  );

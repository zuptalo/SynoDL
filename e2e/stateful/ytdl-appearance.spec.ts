...
test("the sort reaches the YouTube section, in both directions", async ({
  page,
}) => {
  await gotoTasks(page);
  await expect(page.getByTestId("ytdl-item")).toHaveCount(3, {
    timeout: 20_000,
  });

  await setSort(page, "name", true);
  const asc = await ytdlTitles(page);
  expect(asc, "sorting by name did nothing to the YouTube rows").toEqual(
    [...asc].sort(),
  );
...
test("searching for a title a row is showing finds that row", async ({
  page,
}) => {
...
test("a state is drawn as a chip, coloured by what it is doing", async ({
  page,
}) => {
  await gotoTasks(page);
...
function recordThumbRequests(page: Page): () => string[] {
  ...
  void page.route("**/v1/ytdl/thumb**", async (route) => {
    ...
    await route.fulfill({
      status: 404,
      body: "no route to the artwork host in tests",
    });
  });
  return () => asked;
}

test("a row asks for the thumbnail size that has no letterbox bands", async ({
  page,
}) => {
...
test("the detail sheet asks for a sharper image than the row", async ({
  ...
  await expect
    .poll(() => asked().filter((u) => u.includes("maxresdefault")).length, {
      timeout: 20_000,
    })
    .toBeGreaterThan(0);
  ...
  await expect
    .poll(() => asked().filter((u) => u.includes("mqdefault")).length, {
      timeout: 20_000,
    })
    .toBeGreaterThan(0);

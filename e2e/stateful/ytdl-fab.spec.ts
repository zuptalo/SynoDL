...
async function gotoTasks(page: import("@playwright/test").Page): Promise<void> {
  ...
  await expect(page.getByTestId("newtask-fab")).toBeVisible({
    timeout: 30_000,
  });
}
...
test("the YouTube sheet asks for a link and a mode, and nothing else", async ({
  page,
}) => {
  await gotoTasks(page);
  await openFab(page);
  ...
  await expect(page.locator("ion-modal:visible")).not.toContainText(
    "Destination",
  );
...
test("a mode must be chosen before anything can be sent", async ({ page }) => {
  ...
  await page
    .getByTestId("ytdl-url")
    .locator("textarea")
    .fill("https://youtu.be/JDS7zS7_DwE");
  ...
  await expect(page.getByTestId("ytdl-submit")).toHaveAttribute(
    "aria-disabled",
    "true",
  );
  ...
  await expect(page.getByTestId("ytdl-submit")).not.toHaveAttribute(
    "aria-disabled",
    "true",
  );
});

test("submitting from the new sheet puts a row in the Tasks list", async ({
  page,
}) => {
  await gotoTasks(page);
  await openFab(page);
...
test("a link that is not YouTube is refused, in words", async ({ page }) => {
  ...
  await expect(page.getByTestId("ytdl-error")).toContainText(
    /not a supported YouTube address/i,
  );
});
...
test("a non-YouTube link in the general sheet is untouched by any of this", async ({
  page,
}) => {
  await gotoTasks(page);
  await openFab(page);
  ...
  await page
    .getByTestId("newtask-urls")
    .locator("textarea")
    .fill("https://example.com/a.torrent");
...
test("the YouTube sheet can paste a link from the clipboard", async ({
  page,
  context,
}) => {
  ...
  await gotoTasks(page);
  await page.evaluate(() =>
    navigator.clipboard.writeText("https://youtu.be/zSGhyrF7YVo"),
  );

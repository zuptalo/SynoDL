...
let token = "";
const parentFor = (type: string) =>
  type === "series" || type === "anime" ? "/tv-show" : "/movie";

/** Ionic reflects `disabled` as a property, so read the property. */
const sendDisabled = (page: Page) =>
  page
    .locator("ion-modal .send-btn")
    .evaluate((el) => (el as unknown as { disabled: boolean }).disabled);

const chosen = (page: Page) =>
  page
    .locator("ion-modal ion-radio-group")
...
test("a sheet opens with nothing chosen, and cannot be sent", async ({
  page,
}) => {
  ...
  await expect(
    page.locator('ion-modal ion-radio[aria-checked="true"]'),
  ).toHaveCount(0);
});

test("choosing a quality arms the send button and reveals the episodes", async ({
  page,
}) => {
  await openPartlyOwnedSeries(page);
  ...
  await expect(
    page.locator("ion-modal .ep-list ion-checkbox").first(),
  ).toBeVisible();
  await expect(page.locator("ion-modal .send-btn")).toContainText(
    "Send 4 to NAS",
  );
});

test("opening another season drops a choice you can no longer see", async ({
  page,
}) => {
  await openPartlyOwnedSeries(page);
...
test("a season you do not have is sent without being second-guessed", async ({
  page,
}) => {
  await openPartlyOwnedSeries(page);
...
test("a season you already have still asks first", async ({ page }) => {
  ...
  await expect
    .poll(() =>
      page
        .locator("ion-modal ion-accordion-group")
        .evaluate((el) => (el as unknown as { value: string }).value),
    )
    .toBe("1");
  ...
  await page
    .locator("ion-modal ion-accordion")
    .first()
    .locator("ion-radio")
    .first()
    .click();
  ...
  await expect(page.locator("ion-modal .send-btn")).not.toContainText(
    "View in Tasks",
  );
});

test("a movie has no episode picker and sends straight away", async ({
  page,
}) => {

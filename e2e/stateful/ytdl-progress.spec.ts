...
async function emit(requestId: string, line: string): Promise<void> {
  ...
  for (;;) {
    ...
    if (res.ok) return;
    if (res.status !== 404 || Date.now() > deadline)
      throw new Error(`emit: ${res.status}`);
    await new Promise((r) => setTimeout(r, 500));
  }
}
...
test("a running download shows a bar that advances", async ({ page }) => {
  ...
  await emit(
    requestId,
    `${SENTINEL} status=downloading downloaded=20 total=100`,
  );

  await gotoTasks(page);
  await expect(page.getByTestId("ytdl-progress")).toBeVisible({
    timeout: 20_000,
  });
  ...
  await emit(
    requestId,
    `${SENTINEL} status=downloading downloaded=70 total=100`,
  );
  await expect.poll(() => shownPercent(page), { timeout: 20_000 }).toBe(70);
});

test("the bar never goes backwards when a reading arrives out of order", async ({
  page,
}) => {
  ...
  await emit(
    requestId,
    `${SENTINEL} status=downloading downloaded=80 total=100`,
  );
  ...
  await emit(
    requestId,
    `${SENTINEL} status=downloading downloaded=10 total=100`,
  );
  await page.waitForTimeout(6_000);
  const after = await shownPercent(page);
  expect(after, "progress must never move backwards").toBeGreaterThanOrEqual(
    80,
  );
});

test("a download whose output cannot be read shows no bar, and is not failed", async ({
  page,
}) => {
  ...
  await expect
    .poll(
      async () => {
        ...
        return row
          ? `${row.hasLyrics ?? false}/${row.lyricsLang ?? ""}`
          : "missing";
      },
      { timeout: 20_000 },
    )
...
test("a row is the same height with the bar as without it", async ({
  page,
}) => {
  ...
  await emit(
    requestId,
    `${SENTINEL} status=downloading downloaded=40 total=100`,
  );
  await expect(page.getByTestId("ytdl-progress")).toBeVisible({
    timeout: 20_000,
  });

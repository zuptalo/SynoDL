...
async function submit(
  ...
  const body = res.ok
    ? ((await res.json()) as { requestId: string })
    : { requestId: "" };
...
test("a failed download stays failed, even after its job is swept away", async ({
  page,
}) => {
  const { requestId } = await submit(
    token,
    "https://youtu.be/failing",
    "music",
  );
  await gotoTasks(page);
...
test("playlist and channel links are accepted and scoped by the server", async () => {
  ...
  const channel = await fetch(`${API}/v1/ytdl`, {
    ...
    headers: { "Content-Type": "application/json", "X-SynoDL-Session": token },
    body: JSON.stringify({
      url: "https://youtube.com/@someartist",
      mode: "music",
    }),
  });
...
test("the same link cannot be started twice while it is still running", async () => {
  ...
  const asVideo = await submit(
    token,
    "https://youtu.be/onlyonce",
    "music-video",
  );
...
test("a row names the track instead of showing its link", async ({ page }) => {
  ...
  const row = page.getByTestId("ytdl-item").first();
  await expect(row).toBeVisible({ timeout: 15_000 });
  await expect(row.getByTestId("ytdl-name")).toHaveText(
    "Mock Track namedtrack",
  );
 ...
 */
test("a finished download survives its job being swept away", async ({
  page,
}) => {
...
test("a queued download reads differently from one that is starting", async ({
  page,
}) => {
...
test("retry is not offered for a download that has not failed", async ({
  page,
}) => {
  const { requestId } = await submit(
    token,

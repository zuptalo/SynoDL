...
test('the upload flow works from the Tasks tab', async ({ page }) => {
  ...
  // The destination is shown before committing to it.
  ...
  // matters — and it is the destination the SERVER composed, not the preview.
  await expect(page.getByText('movie/Arrival (2016)')).toBeVisible({ timeout: 30_000 });
});

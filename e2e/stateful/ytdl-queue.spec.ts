...
test("a finish frees a slot, whether it succeeded or failed", async () => {
  const ids: string[] = [];
  for (let i = 0; i < 6; i++)
    ids.push(await submit(token, `https://youtu.be/slot${i}`));
  ...
  const running = (await rows(token)).filter(
    (r) => r.state === "scheduled" || r.state === "downloading",
  );
...
test("a queued download can be called off before it ever starts", async () => {
  ...
  const ids: string[] = [];
  for (let i = 0; i < 9; i++)
    ids.push(await submit(token, `https://youtu.be/callable${i}`));

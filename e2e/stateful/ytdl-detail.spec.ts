...
test("tapping a download opens a sheet with everything known about it", async ({
  page,
}) => {
...
test("a download that has not finished shows no finish time, not 1970", async ({
  page,
}) => {
...
test("a failed download explains itself in plain language", async ({
  page,
}) => {
  ...
  for (const forbidden of ["yt-dlp", "/out", "exec", "Traceback", "--"]) {
    expect(reason, `reason must not leak ${forbidden}`).not.toContain(
      forbidden,
    );
  }
});

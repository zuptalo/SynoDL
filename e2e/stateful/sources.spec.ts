...
test("with one source the picker stays hidden and Discover is unchanged", async ({
  page,
}) => {
...
test("two sources are combined by default, interleaved and labelled", async ({
  page,
}) => {
  ...
  const names = await marks.evaluateAll((els) =>
...
test("picking one source narrows the list and drops the labels", async ({
  page,
}) => {
  await addSource(token, "Alpha Source", 0);
  await addSource(token, "Beta Source", 1);
  ...
  await expect(page.locator(".source-select")).toContainText("Beta Source", {
    timeout: 20_000,
  });
...
test("one source failing still shows the other, and says which is missing", async ({
  page,
}) => {
  ...
  await expect(page.locator(".degraded")).toContainText("Html Source", {
    timeout: 30_000,
  });
  // The healthy source is still labelled on its results.
  await expect(page.locator(".src-mark").first()).toHaveAttribute(
    "title",
    "Json Source",
  );
...
test("an admin sees each source and its health, and never a stored secret", async ({
  page,
}) => {
  await addSource(token, "Alpha Source", 0);
  await addSource(token, "Beta Source", 1);

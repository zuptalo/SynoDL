...
async function emitEntries(groupId: string, ids: string[]): Promise<void> {
  ...
  for (;;) {
    const lines = ids
      .map((id) => `${ENTRY} id=${id} uploader=Anyma title=Track ${id}`)
      .join("\n");
...
test("a download changes state on screen without the list being asked again", async ({
  page,
}) => {
  ...
  await emit(
    requestId,
    `${SENTINEL} status=downloading downloaded=60 total=100`,
  );
  await expect(page.getByTestId("ytdl-percent")).toHaveText("60%", {
    timeout: 20_000,
  });
...
test("an idle list asks for nothing at all", async ({ page }) => {
  ...
  expect(
    listFetches(),
    "the list was re-read while nothing was happening",
  ).toBe(0);
});

test("a track inside an open playlist updates without the sheet re-reading it", async ({
  page,
}) => {
  ...
  await expect(
    page.getByTestId("ytdl-group-item").getByTestId("ytdl-percent").first(),
  ).toHaveText("35%", { timeout: 20_000 });

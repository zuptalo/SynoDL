...
test("a title already on the NAS is marked, and its neighbours are not", async ({
  page,
}) => {
  ...
  await seedLibraryFiles({
    [`${parentFor(owned.type)}/${folder}`]: ["episode.mkv"],
  });
...
test("nothing is marked when the NAS holds none of the catalog", async ({
  page,
}) => {
...
test("a near-miss folder name does not mark a title", async ({ page }) => {
  ...
  await seedLibrary({
    [parentFor(subject.type)]: [
      `${folderNameFor(subject.title)} Behind The Scenes`,
    ],
  });
  await refreshLibrary(token, id, "Only Source");
  ...
  const after = await apiSearch(token);
  expect(
    after.find((i) => i.title === subject.title)?.ownership ?? "absent",
  ).not.toBe("owned");
...
test("Discover still works when the parent folders cannot be read", async ({
  page,
}) => {
  const id = await addSource(token, "Only Source", 0);
...
test("a folder holding only metadata is not owned", async () => {
  ...
  const after = await apiSearch(token);
  expect(after.find((i) => i.title === subject.title)?.ownership).toBe(
    "absent",
  );
...
test("a movie reports no seasons", async () => {
  ...
  await seedLibraryFiles({
    [`${parentFor(movie!.type)}/${folder}`]: ["film.mkv"],
  });
  await refreshLibrary(token, id, "Only Source");
...
test("a stored hide-owned preference removes owned titles and survives a reload", async ({
  ...
  await seedLibrary({ [parentFor(owned.type)]: [folder] });
  await seedLibraryFiles({
    [`${parentFor(owned.type)}/${folder}`]: ["film.mkv"],
  });
  await refreshLibrary(token, id, "Only Source");
  expect(
    (await apiSearch(token)).find((i) => i.title === owned.title)?.ownership,
  ).toBe("owned");

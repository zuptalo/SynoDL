...
test("only the release actually on the NAS is marked", async () => {
  ...
  await seedLibraryFiles({
    [base]: [`${slug}.1080p.WEBRip.x264.MockSite.mkv`],
  });
  ...
  // file each produces.
  const otherAt1080 = opts.filter(
    (q) => q.resolution === "1080p" && q.id !== marked[0].id,
  );
  expect(
    otherAt1080.length,
    "there is a same-resolution sibling to confuse",
  ).toBeGreaterThan(0);
  for (const q of otherAt1080) {
    expect(
      q.owned,
      `${q.label} is a different encode and must not be marked`,
    ).toBeFalsy();
  }
...
test("files that do not name a release mark nothing, but the title is still owned", async () => {
  ...
  await seedLibrary({ [parentFor(movie!.type)]: [folder] });
  await seedLibraryFiles({
    [`${parentFor(movie!.type)}/${folder}`]: ["the movie.mkv"],
  });
  await refreshLibrary(token, id, "Only Source");
...
test("a series opens on the first season you do not have, and one at a time", async ({
  page,
}) => {
  const id = await addSource(token, "Only Source", 0);
  const items = await apiSearch(token);
...
test("two options at one resolution are told apart by the file each makes", async () => {
  ...
  const s1 = (detail.qualities ?? []).filter((q) =>
    (q.season ?? "").includes("1"),
  );
  expect(s1.length, "the season offers more than one option").toBeGreaterThan(
    1,
  );
  ...
  for (const q of (detail.qualities ?? []).filter(
    (o) => !(o.season ?? "").includes("1"),
  )) {
...
test("every row fits a narrow screen", async ({ page }) => {
  ...
  await seedLibraryFiles({
    ...
    [`${base}/Season 01`]: Array.from(
      { length: 12 },
      (_, i) => `S.S01E${String(i + 1).padStart(2, "0")}.mkv`,
    ),
  });
  await refreshLibrary(token, id, "Only Source");
  ...
  const overflow = await modal.evaluate((root) => {
    ...
    for (const el of Array.from(
      root.querySelectorAll(
        '[data-testid="season-group"], .quality-row, .season-sub',
      ),
    )) {
      ...
      if (
        ...
        bad.push(
          `${el.className} ${Math.round(r.left)}..${Math.round(r.right)}`,
        );
      }
    }
    return bad;

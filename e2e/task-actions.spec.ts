...
test("pause flips a downloading task to paused on the NAS and in the list", async ({
  page,
}) => {
  await seedTasks([
    {
...
test("resume brings a paused task back to downloading", async ({ page }) => {
  await seedTasks([
    { name: "resumable.iso", status: "paused", size: 1000, downloaded: 10 },
  ]);
  await login(page);
  await expect(page.getByTestId("task-status")).toHaveText("paused");
  await openSliding(page);
...
test("delete requires confirmation; cancel keeps the task, confirm removes it", async ({
  page,
}) => {
  await seedTasks([
    { name: "doomed.iso", status: "paused", size: 1000, downloaded: 10 },
  ]);
  await login(page);
  ...
  await page
    .getByRole("button", { name: "Delete 1", exact: true })
    .last()
    .click();

...
test("header shows NAS-wide speeds from the statistic endpoint", async ({
  page,
}) => {
  await seedTasks([
...
test("empty NAS shows a friendly empty state, not an error", async ({
  page,
}) => {
  await seedTasks([]);
  await login(page);
  // The empty state now speaks for every kind of row, not just NAS downloads.
  await expect(page.getByTestId("tasks-empty")).toContainText(
    "Nothing here yet.",
  );
});

test("a state change on the NAS appears within one polling interval", async ({
  page,
}) => {
  await seedTasks([
    {
  ...
  await seedTasks([
    { name: "living.iso", status: "paused", size: 1000, downloaded: 100 },
  ]);
...
test("a zero-size task (magnet resolving metadata) renders 0% safely", async ({
  page,
}) => {
  await seedTasks([
    {

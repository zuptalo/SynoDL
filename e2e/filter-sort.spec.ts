...
test("the top search bar filters the download list by name", async ({
  page,
}) => {
  await login(page);
  ...
  await expect(page.getByTestId("task-item")).toContainText(
    "medium-paused.zip",
  );
...
test("unchecking a status hides those tasks until re-checked", async ({
  page,
}) => {
  await login(page);
  await expect(page.getByTestId("task-item")).toHaveCount(3);
  ...
  await expect(
    page.getByTestId("task-item").filter({ hasText: "small-old.iso" }),
  ).toHaveCount(0);
});

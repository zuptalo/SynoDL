...
test("wrong password shows a distinct plain-language error", async ({
  page,
}) => {
  await page.goto("/");
...
test("OTP flow: field appears on demand, wrong code distinct, right code succeeds", async ({
  page,
}) => {
...
test("an expired NAS session bounces back to login on the next poll", async ({
  page,
}) => {
  await login(page);

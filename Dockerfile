...
#
# All-in-one SynoDL image: the Go proxy (synodl) serving both the API and the
# built PWA on one origin. Three stages keep the final image small:
#   1. web    - build the Vue/Ionic PWA with Vite into dist/
#   2. server - compile a static synodl binary (CGO disabled, so it runs on alpine)
#   3. runtime- alpine with the binary + dist/, synodl serving the app at / via
#               STATIC_DIR and the API at /v1, /healthz.
#
# Build (multi-arch): docker buildx build --platform linux/amd64,linux/arm64 \
#           -t ghcr.io/zuptalo/synodl:develop --build-arg VERSION=develop --push .
# Run:    docker run -p 8080:8080 -e SYNO_URL=https://nas.local:5001 \
#           ghcr.io/zuptalo/synodl:develop
#
# Stateless BY DESIGN (constitution Principle III): the proxy persists nothing —
# no database, no files, no volumes. Credentials and sessions live on the client.
...
RUN SYNODL_VERSION="$VERSION" \
    SYNODL_RELEASE_NOTES="$([ -n "$RELEASE_NOTES_B64" ] && printf '%s' "$RELEASE_NOTES_B64" | base64 -d || printf '[]')" \
    npm run build
...
# stamped into main.version for `synodl starting version=...` and ops visibility.
...
LABEL org.opencontainers.image.source="https://github.com/zuptalo/synodl" \
      org.opencontainers.image.url="https://github.com/zuptalo/synodl" \
